...
fn main() {
    ...
    fn inspect(noun: &String) {
        if noun.ends_with("s") {
            println!("Noun is plural");
        } else {
            println!("Noun is singular");
        }
    }
    ...
    fn bedazzle(dull_term: &mut String) {
        *dull_term = String::from("sparkly");
    }
}
